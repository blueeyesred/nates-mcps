...
export const metadata = {
  title: "Service Areas | Southern Shine",
  description:
    "Local cleaners serving Oak Ridge, Knoxville, and nearby East Tennessee communities.",
};
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <p className="mt-3 text-muted-foreground max-w-2xl">
        Proudly serving East Tennessee with reliable, respectful cleaning services.
      </p>
      <ul className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-6">
        {AREAS.map((a) => (
          <li key={a.slug} className="rounded-lg border p-5">
            <h2 className="font-semibold">{a.name}</h2>
            <Link href={`/areas/${a.slug}`} className="inline-block mt-2 text-primary underline">
              Learn more
            </Link>
          </li>
        ))}
