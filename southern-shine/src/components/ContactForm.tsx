...
export default function ContactForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setOk(false);
      trackEvent("form_error", { form_id: "contact", error: String(error) });
    } finally {
      setLoading(false);
    }
  }
    ...
    >
      ...
      <div>
        ...
        <input
          id="phone"
          name="phone"
          placeholder="So we can text updates"
          className="mt-1 w-full rounded-md border px-3 py-2"
        />
      </div>
      <div>
        ...
        <textarea
          ...
          rows={5}
          required
          placeholder="Tell us your priorities, pets, or timing."
          className="mt-1 w-full rounded-md border px-3 py-2"
        />
      ...
      {ok === false && (
        <p className="text-red-700">We couldn't send your message. Please try again.</p>
      )}
    </form>
  );
}
