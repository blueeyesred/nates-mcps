...
export default function RequestQuoteForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: Record<string, string | string[]> = Object.keys(entries).reduce(
      (acc, key) => ({
        ...acc,
    ...
    try {
      ...
      setOk(false);
      trackEvent("form_error", { form_id: "quote", error: String(error) });
    } finally {
      setLoading(false);
    }
  }
    ...
    >
      <div>
        ...
        >
          ...
          <option>Weekly Shine</option>
          <option>Deep Reset</option>
          <option>Move-In/Out Clean</option>
          <option>Car &amp; Truck Detailing</option>
        </select>
        {errors.serviceType && (
          <p className="text-sm text-red-600">{errors.serviceType.join(", ")}</p>
        )}
      </div>
      <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            name="name"
            required
            placeholder="We’ll stick to first names if you prefer"
            className="mt-1 w-full rounded-md border px-3 py-2"
          />
        ...
        <div>
          ...
          <input
            ...
            name="phone"
            required
            placeholder="We text when we’re on the way"
            className="mt-1 w-full rounded-md border px-3 py-2"
          />
        ...
        <div>
          ...
          <input
            ...
            name="address"
            required
            placeholder="Oak Ridge, Knoxville, Powell, Clinton…"
            className="mt-1 w-full rounded-md border px-3 py-2"
          />
      ...
      <div>
        ...
        <textarea
          ...
          name="notes"
          rows={4}
          placeholder="Kids, pets, construction dust, or anything else we should know."
          className="mt-1 w-full rounded-md border px-3 py-2"
        />
      </div>
      <button
        type="submit"
        className="inline-flex items-center rounded-md bg-primary px-4 py-2 text-white hover:bg-primary/90 active:scale-[0.98] disabled:opacity-50"
        disabled={loading}
      >
      ...
      {ok === false && (
        <p className="text-red-700">We couldn't submit your request. Please try again.</p>
      )}
    </form>
  );
}
